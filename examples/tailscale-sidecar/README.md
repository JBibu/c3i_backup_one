...
- The `c3i-backup-one` container shares the `tailscale` network namespace (`network_mode: service:tailscale`)
...
```bash
docker exec c3i-backup-one-tailscale tailscale status
docker exec c3i-backup-one-tailscale tailscale ip -4
```
...
```bash
docker exec c3i-backup-one-tailscale ip route
docker exec c3i-backup-one-tailscale ip route show table 52
```