...
services:
  c3i-backup-one:
    image: ghcr.io/nicotsx/c3i-backup-one:v0.22
...
> [!WARNING]
> Do not try to point `/var/lib/c3i-backup-one` on a network share. You will face permission issues and strong performance degradation.
...
| `RESTIC_HOSTNAME`     | The hostname used by Restic when creating snapshots. Automatically detected if a custom hostname is set in Docker. | `c3i-backup-one` |
...
- **Local directories** - Store backups on local disk subfolder of `/var/lib/c3i-backup-one/repositories/` or any other (mounted) path
...
6. **Create a repository** in C3i Backup ONE:
   ...
   - Specify the path within your remote (e.g., `backups/c3i-backup-one`)
...
```bash
# Example
DATABASE_URL=./data/c3i-backup-one.db
